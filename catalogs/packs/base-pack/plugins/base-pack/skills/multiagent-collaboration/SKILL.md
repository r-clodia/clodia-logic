...
description: |
  ...
  Convenzione dei tag: @agente = richiesta diretta (apre un turno), $agente =
  citazione (non apre nulla) — entrambi validi **solo** verso Clodia/Segretario.
---
...
## I due tag, e chi puoi effettivamente menzionare
...
- **`$nome` — CITAZIONE**: non apre nessun turno; il destinatario la legge al
  suo prossimo intervento.

Entrambi i tag, usati da te (uno specialista), sono validi **solo se il
bersaglio è Clodia o Segretario** — per riferire un esito o scalare un
blocco. Non menzionare mai un altro specialista, né con `@` né con `$`: non è
una questione di sigillo giusto/sbagliato, è che quella mention non ti spetta.
...
chiaro, non `@nome`/`$nome`: un resoconto non è una richiesta né una citazione
attiva, è testo che parla di un terzo assente.
...
- Una **citazione** (`$`) non ti attiva: non arriva nessun turno da istruire.
