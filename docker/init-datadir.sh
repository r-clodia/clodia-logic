...
# Migrazioni idempotenti (base-pack diet, ecc.): girano anche qui per coerenza,
# ma il punto di verità è l'entrypoint, che le esegue ad OGNI avvio (init-datadir
# gira solo su datadir vuoto). No-op su un datadir appena seminato.
[ -f "$SCRIPT_DIR/migrate-datadir.sh" ] && bash "$SCRIPT_DIR/migrate-datadir.sh" "$DATADIR" || true
