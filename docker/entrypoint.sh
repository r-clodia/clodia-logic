...
if [ ! -f "/root/.claude.json" ]; then
    # `|| true`: su una claude-home VUOTA (clone/datadir fresco) la dir backups/
    # non esiste → ls fallisce e con `set -euo pipefail` l'entrypoint uscirebbe
    # con 2 (crash-loop). Il fallback va gestito, non deve abortire il boot.
