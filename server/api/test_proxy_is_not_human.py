...
class RunTopicTurnContextTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_the_author_is_sanitised_at_the_shared_point(self) -> None:
        """La riga entra in un PROMPT: la pulizia sta qui, dove passano tutti.

        Sanificare solo nei chiamanti significa che il prossimo chiamante
        reintroduce l'iniezione senza che nessuno se ne accorga.
        """
...
class HookTriggerTests(unittest.TestCase):
    """L'ALTRA porta da cui un sistema terzo apre un turno: il webhook.

    `server/hooks/api.py` ha due ingressi con due autorità diverse, e la
    provenienza deve seguire quella, non il nome:

    - `POST /hooks/{id}` è autorizzato da un SEGRETO. Non c'è nessuna identità
      da classificare, e `principal` è la costante `"hook"`.
    - `POST /clodia/hooks/{tier}/{name}/invoke/internal` è autorizzato da un
      session token FIRMATO: lì il nome è provato e si può classificare.
    """

    def _queued(self, fn) -> dict:
        ...
        with patch.object(hooks_api.topics_client, "open_topic",
                          return_value={"meta": {"tier": "SEAL-1"}}), \
             patch.object(ch, "_spawn_bg", side_effect=lambda coro: coro.close()), \
             patch.object(ch, "run_topic_turn", new=_fake_turn):
            self.assertTrue(fn(hooks_api))
        return visto

    def test_the_public_ingress_is_external_by_construction(self) -> None:
        """IL punto del finding 5: `"hook"` è un NOME, non una prova.

        Il registry qui risolve `hook` a una PERSONA — configurazione ostile ma
        del tutto legale. Se la provenienza si ricostruisse dal nome, un webhook
        pubblico entrerebbe come `human` per fortuna del naming.
        """
        import asyncio
        ...
        req = type("R", (), {})()
        req.headers = {"X-Hook-Secret": "s"}
        req.query_params = {}
        req.client = None
        req.body = _raw

        row = {"tier": "SEAL-1", "name": "ops", "author": "hook", "rate_per_min": 30}
        with patch.object(ch.registry, "get_by_name",
                          side_effect={"hook": _a("hook", "human")}.get):
            # la trappola, esplicita: ricostruire dal nome darebbe `human`…
            self.assertEqual(hooks_api._signed_kind("hook"), "human")
            # …e l'ingress pubblico non ci casca, perché non ricostruisce.
            with patch.object(hooks_api.db, "verify_secret", return_value=row), \
                 patch.object(hooks_api.db, "record_event"), \
                 patch.object(hooks_api.topics_client, "post_message"), \
                 patch.object(hooks_api, "_rate_ok", return_value=True):
                visto = self._queued(
                    lambda h: asyncio.run(h.ingress("h1", req))["triggered"])
        self.assertEqual(visto.get("trigger_kind"), "external")
        self.assertEqual(visto.get("trigger_author"), "hook")

    def test_a_signed_person_on_the_local_hook_stays_human(self) -> None:
        """L'ingresso locale verifica il token prima di arrivare qui: su
        un'identità provata la classificazione dal nome è legittima."""
        with patch.object(ch.registry, "get_by_name", side_effect=AGENTS.get):
            from ..hooks import api as hooks_api
            self.assertEqual(hooks_api._signed_kind("davide"), "human")
            self.assertEqual(hooks_api._signed_kind("clodia"), "ai")
            self.assertEqual(hooks_api._signed_kind("clodia-primal"), "external")

    def test_an_unusable_registry_degrades_to_external(self) -> None:
        """Fail-closed come il resto di #221: nel dubbio non è una persona."""
        with patch.object(ch.registry, "get_by_name",
                          side_effect=RuntimeError("registry non caricato")):
            from ..hooks import api as hooks_api
            self.assertEqual(hooks_api._signed_kind("davide"), "external")
